...
import { 
    ...
    Eye, Trash2, Search, ChevronDown, LogOut, Home, BadgeCheck, Clock, Zap, XCircle
} from 'lucide-react';
...
function AdminPage() {
    ...
    useEffect(() => {
        const checkAdminAndFetch = async () => {
            try {
                ...
                if (listingsResult.ok) setListings(listingsResult.data.data || []);
                if (usersResult.ok) setUsers(usersResult.data.data || []);
            } catch (error) {
    ...
    const loadVerificationRequests = async (status = verificationFilter) => {
        const result = await verificationService.adminGetRequests(status);
        if (result.ok) {
            setVerificationRequests(result.data.requests || []);
            setVerificationStats(result.data.stats || { pending: 0, approved: 0, rejected: 0 });
        }
    };

    const handleViewDocuments = async (requestId) => {
        const result = await verificationService.adminGetDocuments(requestId);
        if (result.ok) {
            setViewingDocuments(result.data);
        } else {
    ...
    const handleApprove = async (requestId) => {
        if (!confirm('Certifier cet utilisateur ?')) return;
        const result = await verificationService.adminApprove(requestId);
        if (result.ok) {
    ...
    const handleReject = async (requestId) => {
        ...
        const result = await verificationService.adminReject(requestId, rejectReason);
        if (result.ok) {
            alert(result.data.message);
                            ...
                            <>
                                <div className="flex items-center justify-between mb-8">
                                    ...
                                    <div className="flex items-center gap-3">
                                        <span className="text-sm text-gray-500">
                                            🟡 {verificationStats.pending} en attente · ✅ {verificationStats.approved} certifiés · ❌ {verificationStats.rejected} rejetés
                                        </span>
                                    </div>
                                </div>

                                {/* Filtres + Certification manuelle */}
                                <div className="flex flex-wrap items-center gap-4 mb-6">
                                    <div className="flex bg-white rounded-lg overflow-hidden border border-gray-200">
                                        {[
                                            { id: 'pending', label: 'En attente', icon: Clock },
                                            { id: 'approved', label: 'Certifiés', icon: Check },
                                            { id: 'rejected', label: 'Rejetés', icon: XCircle },
                                            { id: 'all', label: 'Tous', icon: Eye },
                                        ].map(f => (
                                            <button
                                                key={f.id}
                                                onClick={() => { setVerificationFilter(f.id); loadVerificationRequests(f.id); }}
                                ...
                                {verificationRequests.length === 0 ? (
                                    <div className="bg-white rounded-2xl p-12 text-center">
                                        ...
                                        <p className="text-gray-500">Aucune demande {verificationFilter === 'pending' ? 'en attente' : ''}</p>
                                    </div>
                                ) : (
                                                ...
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div className="flex-1">
                                                        ...
                                                        <p className="text-sm text-gray-500">
                                                            {vr.user?.email} · Catégorie : <strong>{vr.category}</strong> · Tentative {vr.attemptNumber}/3
                                                        </p>
                                                        <p className="text-xs text-gray-400 mt-1">
                                                            Soumis le {new Date(vr.createdAt).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
                                                            {vr.reviewedAt && ` · Traité le ${new Date(vr.reviewedAt).toLocaleDateString('fr-FR')}`}
                                                        </p>
                                                        {vr.rejectionReason && (
                                                            <p className="text-xs text-red-600 mt-1">Motif : {vr.rejectionReason}</p>
                                                        )}
                                                    </div>
                                                    <div className="flex items-center gap-2">
                                                        <button
                                ...
                                {viewingDocuments && (
                                    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                                        <div className="bg-white rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto p-6">
                                            ...
                                            <p className="text-sm text-gray-500 mb-4">Catégorie : <strong>{viewingDocuments.category}</strong></p>
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                {viewingDocuments.documents?.map((doc, idx) => (
                                                    <div key={idx} className="border border-gray-200 rounded-xl p-4">
                                                        <p className="font-medium text-gray-700 mb-2">{doc.key}</p>
                                                        {doc.error ? (
                                                            <p className="text-red-500 text-sm">{doc.error}</p>
                                                        ) : doc.mimeType?.startsWith('image/') ? (
                                                            <img src={doc.data} alt={doc.key} className="w-full rounded-lg max-h-64 object-contain bg-gray-50" />
                                                        ) : (
                                                            <a href={doc.data} download={doc.key} className="text-blue-500 hover:underline text-sm">
                                                                Télécharger le PDF ({Math.round(doc.size / 1024)}KB)
                                                            </a>
                                                        )}
                                                    </div>
                                                ))}
